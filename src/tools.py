...
def analyze_alter_ego(traits: str) -> str:
    ...
    try:
        ...
        # TRÁNH CRASH CODE: Bắt mọi lỗi ngoại lệ và trả về chuỗi JSON thông báo lỗi
        return json.dumps({"error": f"Lỗi nội bộ tool (analyze_alter_ego): {str(e)}"}, ensure_ascii=False)
...
def get_counseling_action_plan(alter_ego_id: str, risk_level: str) -> str:
    ...
    try:
        ...
        # Đảm bảo alter_ego_id là string hợp lệ để truy xuất từ điển
        safe_id = str(alter_ego_id) if alter_ego_id is not None else ""
        selected_plan = plans.get(safe_id, default_plan)
        
        response = {
            "action_plan": selected_plan,
            "note": "Cần chú ý theo dõi sát sao do mức độ rủi ro cao." if str(risk_level) == "High" else "Hãy thực hành từ từ theo nhịp độ của bản thân."
        }
        
        return json.dumps(response, ensure_ascii=False)
    except Exception as e:
        # TRÁNH CRASH CODE
        return json.dumps({"error": f"Lỗi nội bộ tool (get_counseling_action_plan): {str(e)}"}, ensure_ascii=False)
